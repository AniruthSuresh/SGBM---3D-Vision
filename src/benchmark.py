        ...
        for d in range(0,disparity):        
            for i in range(h):
                for j in range(w):
                    ...
                    # Sum the elements in the grid and store in result
...
def get_accuracy(disparity, gt, args):
    """
...
def benchmark_sgbm(left_name="../data/cones/im2.png",
                right_name="../data/cones/im6.png" , 
                type = "cone"):
    """
    ...
    for grid in range(1, 6, 4):
        ...
        image_left.append(left_disparity_map)
        image_right.append(right_disparity_map)
    ...
    disparity_left = stereo.compute(left, right)
    disparity_left_normalized = cv2.normalize(disparity_left, disparity_left, 0, 255, cv2.NORM_MINMAX)
    disparity_left_normalized = np.uint8(disparity_left_normalized)

    """
    This is the TRICKY part !! -> to compute the left disp using the opencv -> we need to change the min_disp to negative to account for left 
    else , we get a crappy output .
    """
    ...
    disparity_right = stereo.compute(right, left)
    disparity_right = -disparity_right
    disparity_right_normalized = cv2.normalize(disparity_right, disparity_right, 0, 255, cv2.NORM_MINMAX)
    disparity_right_normalized[disparity_right_normalized >= 252] = 0
    disparity_right_normalized = np.uint8(disparity_right_normalized)

    image_left.append(disparity_left_normalized)
    image_right.append(disparity_right_normalized)

    n_images = len(image_left)
    rows = 1
    cols = n_images 
    fig, axs = plt.subplots(rows, cols, figsize=(15, 8))
    axs = axs.flatten()

    for i, image in enumerate(image_left):
    ...
    plt.tight_layout()
    plt.savefig(f"../results/benchmark_{type}_left.png")
    plt.show()

    n_images = len(image_right)
    fig, axs = plt.subplots(rows, cols, figsize=(15, 8))
    axs = axs.flatten()

    for i, image in enumerate(image_right):
        axs[i].imshow(image, cmap='gray')
        axs[i].axis('off') 
    ...
    plt.tight_layout()
    plt.savefig(f"../results/benchmark_{type}_right.png")
...
benchmark_sgbm(left_name=left_name , right_name= right_name , type = "kitti")
