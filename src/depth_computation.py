...
left_name="../data/kitti_reduced/2011_09_26/2011_09_26_drive_0001_sync/image_00/data/0000000000.png"
right_name="../data/kitti_reduced/2011_09_26/2011_09_26_drive_0001_sync/image_01/data/0000000000.png"
process_stereo_images(left_name=left_name , right_name= right_name , evaluation=False , type = "kitti")
...
def process_stereo_images(left_name="../data/cones/im2.png", right_name="../data/cones/im6.png",
                          left_gt_name='../data/cones/disp2.png', right_gt_name='../data/cones/disp6.png',
                          output_name='disparity_map_sgbm.png', disparity=64,
                          save_images=False, evaluation=True , type = "cones"):
    """
    This is the main wrapper function which calls all the other function 
    """
    ...
    print(left_disparity_map)
    right_disparity_map = cv2.medianBlur(right_disparity_map, parameters.bsize[0])

    cv2.imwrite(f'../results/left_{type}_{output_name}', left_disparity_map)
    cv2.imwrite(f'../results/right_{type}_{output_name}', right_disparity_map)

    if evaluation:
        print('\nEvaluating left disparity map...')
        accuracy = get_accuracy(left_disparity_map, left_gt_name, disp = disparity)
        accuracy_left.append(accuracy)
        print('\taccuracy = {:.2f}%'.format(accuracy * 100.0))
        

        print('\nEvaluating right disparity map...')
        accuracy = get_accuracy(right_disparity_map, right_gt_name,disp = disparity)
    ...
    time_elapsed.append(dusk - dawn)

    return left_disparity_map

def get_the_intrinsics_matrix():
    """
    This function gets the intrinsics matrix of the current data 
    """

    """
    Currently corresponding to -> /traning/calib/0000000.txt
    """
...
def disparity_to_pointcloud(disparity, img, scale=1.0):
    """
    ...
    for v in range(h):
        for u in range(w):
            ...
            if disparity[v, u] > 0:  # Ignore invalid disparity values
                z = depth[v, u]
                x = (u - cx) * z / fx
                y = (v - cy) * z / fy

                pc_points.append([x, y, z])
                pc_colors.append(img[v, u] / 255.0)  
...
# run this for kitti 
left_name="/home/aniruth/Downloads/data_stereo_flow/training/image_0/000000_10.png"
right_name="./home/aniruth/Downloads/data_stereo_flow/training/image_1/000000_10.png"

disparty_left = process_stereo_images(left_name=left_name , right_name= right_name , evaluation=False , type = "kitti")
